...
const [torso,parts,boots,torsoSE]=await Promise.all(['torso_parts.png','leg_parts.png','foot_parts.png','torso_se.png'].map(load)).then(a=>a.map(keyed));
...
function drawLeg(leg,dir){
  ...
  segment(h,k,[pw*.34,ph*.02,pw*.40,ph*.94],ph*.09,ph*.83,24);
  segment(k,a,[pw*1.25,ph*.035,pw*.47,ph*.92],ph*.075,ph*.815,18);
  ...
  ctx.drawImage(boots,source%4*cw,Math.floor(source/4)*ch,cw,ch,-footAnchors[source]*34,-.85*24,34,24);ctx.restore();
}
function drawTorso(p,dir){
  ...
  if(source===7){const w=size*torsoSE.width/torsoSE.height;ctx.drawImage(torsoSE,-.41*w,-.865*size,w,size);}
  else ctx.drawImage(torso,col*tcw,row*tch,tcw,tch,-anchor[0]*size,-anchor[1]*size,size,size);ctx.restore();
}
...
export function render(phase,options={}){
  ...
  if(selected!=='all'){
    ...
    ctx.fillStyle='#f4cf55';ctx.font='bold 19px system-ui';ctx.fillText('ENGINEER · IK',320,352);
