	...
	for id in ["hunter", "veteran", "engineer"]:
		...
		if id in ["hunter", "veteran", "engineer"]:
			...
			if id in ["hunter","engineer"]:
				check(p._walk_muzzles[7].y-p._walk_muzzles[6].y>20, "SE muzzle follows lowered diagonal weapon")
				var atlas: Image = p._sheet_tex.get_image()
				check(atlas.get_region(Rect2i(0,6*128,3072,128)).get_data()!=atlas.get_region(Rect2i(0,7*128,3072,128)).get_data(), id+" E and SE rows duplicated")
				check(atlas.get_region(Rect2i(0,2*128,3072,128)).get_data()!=atlas.get_region(Rect2i(0,1*128,3072,128)).get_data(), id+" W and SW rows duplicated")
		else:
