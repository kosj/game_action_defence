...
PLAN = {
    ...
    # (플랫니스 0.028, 9.5배 노이즈다움). 0.05~1.25s 의 한 프레이즈만 써서 레벨업(2.0s)의
    # 절반 길이로 만든다 — 음색·길이 두 축 모두에서 갈라놓는다.
    # 2.2s 구간을 쓴다 — 창을 옮겨가며 비교했을 때 가장 노이즈다웠고(플랫니스 0.0143 =
    # 레벨업의 2.1배) 중심 주파수도 763Hz 로 가장 낮아 레벨업(1746Hz)과 멀다. 250ms 스웰로
    # 시작해 '몰려온다'는 느낌도 살아난다(앞부분 0.0s 구간은 어택은 세지만 음색이 덜 갈린다).
