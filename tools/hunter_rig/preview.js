...
function drawTorso(p,dir){
  ...
  if(source===7) ctx.drawImage(torsoSE,-.45*size,-.90*size,size,size);
  else ctx.drawImage(torso,col*tcw,row*tch,tcw,tch,-anchor[0]*size,-anchor[1]*size,size,size);
  ctx.restore();
}
...
export function render(phase,options={}){
  ...
  if(selected!=='all'){
    ...
    ctx.fillStyle='#82dec5';ctx.font='bold 19px system-ui';ctx.fillText('HUNTER · IK',335,352);
