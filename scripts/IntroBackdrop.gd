extends Control
## 인트로 "Hold the Ground" 분위기 배경 — 코드로 그린 일러스트.
##
## 톤은 타이틀 아트(assets/ui/bg_title.png)를 그대로 이어받는다: 크림슨 노을이 지평선에서
## 타오르고, 그 앞을 폐허 도시 실루엣 3겹이 가리며, 잔불(ember)이 천천히 떠오른다.
## 그 위에 송신탑(맥동 비컨)이 서 있다 — 인트로 서사의 주인공.
## 구조 요청이 아니라 "그 구역을 30분 지켜내면 켜지는 표식"이다(intro_l4).
##
...
const BEACON_COL := Color(1.0, 0.36, 0.26)   # 점령 표식(붉은) 비컨 색
