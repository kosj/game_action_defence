...
const STRINGS: Dictionary = {
	...
	"pickup_heal":     {"en": "First Aid", "ko": "체력 회복", "ja": "かいふく"},
	...
	# ── 영구 강화(파워업) 행 ──────────────────────────────────────────────
	# 레벨과 가격은 이제 글자가 아니라 위젯이다(레벨 핍 + 코인 가격표) — 남은 문구는
	# 만렙 표시 하나뿐이다. 이름·설명은 카탈로그에서 오므로 번역 대상이 아니다.
	"power_max_tag": {"en": "MAX", "ko": "최대", "ja": "さいだい"},
	# 예전에는 양옆에 "!!" 를 붙여 위험을 표시했다. 지금은 띠 양끝의 경고 삼각형이 그
	# 일을 하므로 뺐다 — 아스키 장식은 폰트·언어마다 다르게 읽히고, 같은 말을 두 번 한다.
