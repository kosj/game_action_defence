...
## ⚠️ 원·쐐기·호는 **QuadDraw(텍스처 쿼드)** 로 그린다 — 프리미티브를 직접 쓰지 않는다.
## 원 3 + 쐐기 9 + 호 2 가 전부 다른 프리미티브라 서로 안 묶였다(실측 마늘 +91).
func _draw() -> void:
	var r := _radius()
	var breathe := 1.0 + 0.03 * sin(_pulse * 3.0)
	var rr := r * breathe
	# 다층 금빛 글로우 — 바깥으로 갈수록 옅어지는 햇살 웅덩이.
	QuadDraw.disc(self, Vector2.ZERO, rr, Color(1.0, 0.85, 0.45, 0.07))
	QuadDraw.disc(self, Vector2.ZERO, rr * 0.72, Color(1.0, 0.90, 0.55, 0.07))
	QuadDraw.disc(self, Vector2.ZERO, rr * 0.45, Color(1.0, 0.95, 0.70, 0.08))
	...
	for i in RAYS:
		...
		QuadDraw.wedge(self, Vector2.ZERO, ang, half, rr, Color(1.0, 0.92, 0.60, 0.06))
	# 가장자리 림 — 은은한 노을빛 경계 + 반짝이는 하이라이트 호.
	QuadDraw.ring(self, Vector2.ZERO, rr, Color(1.0, 0.82, 0.40, 0.30), 2.5, 56)
	var hl := fmod(_pulse * 0.7, TAU)
	QuadDraw.ring(self, Vector2.ZERO, rr, Color(1.0, 0.97, 0.80, 0.45), 3.0, 14, hl, hl + 0.9)
