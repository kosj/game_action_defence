...
const KEYS: Array = [
	[0.00, Color(0.67, 0.71, 0.95)],   # 한밤 — 가장 어두움(= 보스 등장 시각).
	                                   # 초판(0.62,0.66,0.92 = 휘도 0.67)은 캐릭터가 안 보인다는
	                                   # 피드백이 있어 휘도 0.72 로 올렸다 — 어둡되 실루엣은 살게.
	...
	[1.00, Color(0.67, 0.71, 0.95)],   # 한밤(= 0.00)
]

## 가독성 하한. 바닥은 이미 Ground.TILE_DARKEN(0.55)으로 깔려 있어, 여기서 이 밑으로 더 내리면
## 좀비 실루엣과 탄이 죽는다. 날씨 틴트까지 곱한 "최종" 색에 적용한다.
## (날씨가 더 이상 색을 입히지 않으므로 현재 실플레이에서는 발동하지 않는다 — _weather_tint 참고)
## 0.62 → 0.66: 밤 가독성 피드백으로 한밤 틴트와 함께 올렸다 — 밤×비(최악 조합)의 실효 휘도를 지킨다.
const LUMA_FLOOR := 0.66
...
## 반경·세기는 밤 가독성 피드백으로 한 번 올린 값(460/0.18 → 520/0.28) — 캐릭터와 근접 전투
## 반경이 밤에도 또렷해야 한다. 가산 쿼드 1장이라 키워도 비용은 그대로다.
const HALO_RADIUS := 520.0
...
const NIGHT_TO := 0.72   # 새 한밤 휘도(0.719)에 맞춘다 — 한밤에 밤 강도가 정확히 1.0 에 닿는다
...
## 치트가 끄는 것은 "시간"뿐이라 비/눈 입자는 그대로 떨어진다(날씨는 애초에 색을 입히지 않는다).
...
##
## ⚠️ **지금은 아무도 써 넣지 않는다.** 비/눈이 배경을 어둡게 물들이던 것을 실플레이 피드백으로
## 걷어냈다(WeatherSystem 주석 참고) — 날씨는 입자로만 보여 준다. 이 입력과 아래 가독성 하한은
## 색을 입히는 날씨가 다시 들어올 때를 위한 **자리**로 남겨 둔 것이고, 검증이 합성 색으로
## 계속 확인한다. 지금 실제 값은 항상 흰색이라 하한 보정은 발동하지 않는다.
