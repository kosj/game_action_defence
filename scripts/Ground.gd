...
const TILE := 80
## 바닥 타일에 곱하는 감광. 그 위 이펙트가 잘 보이게 하려는 것인데, **너무 세면 정반대가 된다.**
##
## 0.55 → 0.80 (실측 근거). 잔디 원본 휘도가 0.478 인데 0.55 를 곱하면 0.263 이고, 이건
## 플레이어 스프라이트의 화면 휘도(0.2604)와 **사실상 같은 값**이다 — 마이컬슨 대비 0.0006,
## 즉 유닛이 바닥과 같은 밝기라 색상 차이만으로 겨우 구분됐다("바닥이 선명하지 않다" 제보).
## 감광은 이펙트를 돕는 대신 유닛을 배경에 묻고 있었다.
##
## 값별 실측(한낮·교전 중, 바닥 영역 평균 휘도 / 유닛-바닥 마이컬슨 대비):
##   0.55  0.261 / 0.0006   ← 유닛이 안 보인다
##   0.70  0.322 / 0.0244
##   0.80  0.363 / 0.0366   ← 채택. 20% 는 여전히 감광이라 밝은 이펙트의 여유가 남는다
##   0.90  0.404 / 0.0463   ← 감광의 목적이 거의 사라진다
## 밤에는 CanvasModulate 가 한 번 더 곱해진다(한밤 0.72) — 0.478×0.80×0.72 = 0.275 로,
## 예전의 한낮 밝기 언저리다. 파랑을 조금 높게 둔 것(0.85)은 원래의 차가운 색조 유지.
const TILE_DARKEN := Color(0.80, 0.80, 0.85)
