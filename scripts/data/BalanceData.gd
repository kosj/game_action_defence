...
## 강화 카드 뽑기 가중치 — 이미 올린 아이템이 다시 뜰 확률을 레벨에 비례해 높인다.
## 후보 하나의 가중치 = 1 + 현재레벨 × 이 값 (미보유 아이템은 레벨 0 이므로 항상 1).
##
## 왜 필요한가 — 예전에는 후보를 **균등하게 섞어** 3장을 뽑았다. 슬롯이 차면(무기 6 + 패시브 6)
## 후보가 12개라 특정 아이템이 뜰 확률이 25% 이고, 무기 하나를 Lv1→8(진화 조건)로 올리려면
## 평균 **28번의 레벨업**이 필요했다 — 캐릭터 레벨 40 근처다. 실측 36판에서 Lv8 에 닿은 판은
## 2판뿐이고 **진화는 0판**이었다(사람 기록도 레벨 62 판 하나에서만 나왔다).
## 즉 진화 11종이 "판이 끝나는 지점 너머"에 있었다. 집중을 **선택**으로 만들기 위한 값이다.
##
## 0 = 예전 동작(균등). 올릴수록 집중 빌드가 빨리 완성된다.
@export var levelup_focus_weight: float = 1.0
