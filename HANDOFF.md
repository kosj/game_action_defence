...
| P1-22 분리 패스 최적화 검토 + `only=` 귀속 결함 수정 | I | 🔵 진행중 | claude/gpu-cpu-bottleneck-optimization-auaaun | 2026-08-20 |
...
- **`ZombieSpawner` 의 좀비 분리 패스**가 이제 가장 크다(`only=` 기준 3.83ms, 좀비 320 기준).
  개체 수에 선형이라 **P1-20(`max_z_cap` 낮추기)과 직접 맞물린다** — P1-20 을 하면 같이 줄어든다.
