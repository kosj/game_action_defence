...
| P0-8 패시브가 기록에 안 남는다(획득 불가 의심) | F | ⚪ 대기 | — | 2026-08-20 |
...
## P0-8. 최근 라이브 판에 패시브가 **한 개도** 기록되지 않았다 🔴

**근거** — 사용자 제공 플레이 기록(2026-08-20 수집분) 11판을 빌드별로 가르면 경계가 선명하다.

| 빌드 | 판수 | 패시브 종류 수 | 무기 종류 수 |
|---|---|---|---|
| `60f0683` · `80082c8` · `d8ccfaa` · `2ebbaa2` | 6 | 3~6 | 6~7 |
| `cb3a86b` · `9b79bdc` | **5** | **전부 0** | 6~7 |

문제가 되는 판:
- **레벨 128 · 24.3분** — 무기 7종의 레벨 합계가 48뿐이다. 나머지 **약 80회의 카드 선택이 어디로도
  안 갔다.** 레벨 88·61·52 판도 같다.
- 무기 슬롯이 6칸이라 후반에는 **뽑기 후보 대부분이 패시브**여야 한다(`LevelUpPanel._draw_choices`).
  그런데 패시브가 0이면, 카드에 아예 안 떴거나 골랐는데 인벤토리에 안 들어갔다는 뜻이다.

**영향** — 둘 중 어느 쪽이어도 심각하다.
- ① *패시브를 못 얻는다* → 패시브 10종 + **진화 12종이 통째로 도달 불가**(진화 조건이 짝꿍 패시브 보유다).
- ② *기록만 누락* → 사람 데이터가 밸런스 판단에 못 쓰인다. P1-5 가 실제로 이것 때문에 결론을 못 냈다.

**⚠️ 헤드리스에서는 재현되지 않는다.** 현재 `main` 에서 확인한 것:
- `GameData.passive_defs` 10종 정상, `ItemDB.meta()` 전부 조회됨
- `sim_balance` 14분 판에서 패시브 6종 정상 기록
- 세이브 왕복(`save_game` → `load_save` → `apply_to_events`) 후 패시브 보존됨
- `9b79bdc..main` 사이에 아이템/세이브 관련 커밋 없음 (텔레메트리 2건 + 문서뿐)

즉 **코드 경로가 아니라 웹 빌드/실제 조작 쪽에 원인이 있다.** 5판 중 4판이 `resumed=true` 라
이어하기 경로가 첫 용의자다(단, `d8ccfaa` 의 이어하기 판은 패시브가 정상이었다).

**작업**
1. **웹 빌드에서 직접 재현한다.** 한 판 시작 → 레벨업 카드에 패시브가 뜨는지 눈으로 확인 →
   하나 고른 뒤 HUD 인벤토리에 들어오는지 확인. 이어하기 판으로도 같은 절차를 반복한다.
   (헤드리스로는 안 잡힌다 — 위 확인이 이미 전부 통과한다.)
2. 카드에 안 뜬다면 `ItemDB.passives()` / `_collect` 의 슬롯 판정, 들어오는데 사라진다면
   `Events.grant_item` → `ItemDB.recompute` → 세이브 왕복 순으로 좁힌다.
3. 원인이 잡히면 **회귀 테스트를 먼저 쓴다** — 이어하기 판에서 패시브를 얻고 저장·복원까지
   왕복하는 케이스를 `ContinueSaveTest.tscn` 에 추가한다.

**수용 기준** — 웹 빌드에서 한 판을 돌려 패시브가 카드로 등장하고, 획득 후 기록의 `passives` 에
남는다. 이어하기로 재개한 판에서도 같다.

**검증** — `scenes/ContinueSaveTest.tscn` · `scenes/TelemetryTest.tscn`
