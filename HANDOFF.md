...
| P2-8 오토플레이 교전 이탈 수정 | F | 🔵 진행중 | claude/f-lane-autoplay-engage | 2026-08-18 |
| P2-1 공통 팝업 셸 | D | ⚪ 대기 | — | — |
...
`Cheats.gd` 의 반발 반경 `_AVOID_R`(200)·`_BOSS_R`(360) 과 교전 반경 `_ENGAGE_R`(300) 이
서로를 밀어내, 플레이어(220)가 좀비(65~150)보다 빨라 영원히 도망칠 수 있는 상태가 된다.

**영향** — 측정 판의 20~40%가 버려진다(`sim_balance.py` 가 분당 처치 40 미만을 이탈로 제외).
측정 효율이 직접 깎이고, 치트로 게임을 눈으로 확인할 때도 같은 증상이 나온다.

**작업** — 반발 세기를 거리 기반으로 부드럽게 하거나, 무기 사거리 안에 적을 유지하는 항을 추가한다.
목표는 "사람처럼"이 아니라 **"교전은 한다"** 수준이면 충분하다.

**수용 기준** — `sim_balance.py` 12판에서 이탈 판정이 1판 이하.
