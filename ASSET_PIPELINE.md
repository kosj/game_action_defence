...
> 해당한다. 실제로 `ui_coin`·`ui_heart_full`·`ui_heart_empty`(각 48×48)가 이 분류에 휩쓸려
> 아틀라스 밖에 남았는데, **HUD 에서 매 프레임 그려지는 작은 아이콘**이라 텍스처 교체가
> 그대로 드로우 콜이 된다(HUD 격리 실측: 115 items → 40 draw, 배칭비 2.9:1).
> 판단 기준은 위치가 아니라 **"매 프레임 다른 것들과 섞여 그려지는가"** 다.
