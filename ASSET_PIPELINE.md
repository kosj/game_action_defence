...
### 월드에 뜨는 텍스트는 폰트가 아니라 구운 비트맵으로

폰트 글리프 아틀라스는 게임플레이 시트와 **별개 텍스처**다. `draw_string` 을 부르는 순간
그 자리에서 배치가 끊기고, 아웃라인(`draw_string_outline`)은 한 번 더 끊는다.
데미지 숫자가 정확히 그랬다 — 노드 하나당 2회 × 동시 36개 = 실측 **드로우 콜 49개**.

글자 종류가 유한하면(숫자 0~9 처럼) **미리 구워서 아틀라스에 넣는다.**

```bash
xvfb-run -a godot --path . --rendering-driver opengl3 \
    --script res://tools/gen_damage_digits.gd     # assets/sprites/dmg_0..9.png
python3 tools/build_atlas.py
```

- **검은 아웃라인 + 흰 글리프를 한 장에** 굽는다. 쓰는 쪽에서 색을 틴트로 곱하면
  아웃라인은 `0 × c = 0` 으로 검정 그대로고 글리프만 `1 × c = c` 로 색이 된다 —
  텍스처 한 장이 원본의 두 번 드로우를 대신한다.
- **글리프는 Godot 이 직접 렌더한다**(PIL 로 다시 그리지 않는다). 폰트가 같아야 모양이 같다.
- **10장을 공통 사각형으로 자른다.** 장마다 따로 자르면 글자마다 세로 위치가 어긋나
  숫자열이 출렁인다. 동시에 폰트 전체 높이(55×98)의 빈칸이 사라진다.
- 전진폭은 **표시 크기에서 직접** 잰다. 큰 크기에서 재고 나누면 힌팅 반올림 때문에
  원본과 문자열 폭이 미세하게 달라진다.
- ⚠️ **`--headless` 로는 못 굽는다.** 더미 렌더러라 `SubViewport` 캡처가 null 이다.
  실제 GL 컨텍스트(xvfb + llvmpipe)가 필요하다.

완전 재현은 아니다 — 원본 아웃라인은 폰트 크기와 무관하게 5px 고정인데 텍스처는 같이
확대된다. 그래서 기본 크기(24pt)에서 정확히 5px 가 되도록 굽고, 크리티컬 팝에서는 조금
두꺼워지는 것을 **의도된 근사**로 받아들였다(나란히 렌더해 비교했고 가독성은 오히려 낫다).

임의 문자열(무기 이름 같은 로케일 텍스트)은 구울 수 없다. 그건 **동시 표시 수를 한 자릿수로
묶는 것**이 답이고, `verify_batching.gd` 에 `# batching-exempt: <이유>` 로 근거를 남긴다.
이 검사는 `HOT_DRAW` 목록의 스크립트에서 문자열 드로우를 **루프 밖이어도** 잡는다 —
배수가 루프가 아니라 인스턴스 수이기 때문이다.
