...
> 베테랑·엔지니어는 기존 정지 그림 + 절차 걷기를 유지한다. 아래 본문은 이전 3종 시트 작업 기록이다.
...
세 캐릭터 걷기 시트는 **완료**됐지만 지금은 쓰이지 않는다(1절 참고).

1. **시트를 다시 쓰려면 프레임간 색 일관성을 더 잡아야 한다.** 팔레트 편차 16~22 로는
   프레임이 넘어갈 때 바지·장비 색이 흔들리는 게 보인다. 2단계 정규화로 42.8 → 19.1 까지
   줄였지만 그 이상은 생성 방식으로 해결이 안 됐다(전역 색 보정은 오히려 악화 — 4-5절).
   손으로 팔레트를 고정하거나, 한 프레임을 골라 다리만 그려 넣는 편이 현실적이다.
2. 엔지니어의 인접 실루엣차가 33.8% 로 셋 중 가장 낮다(기준 25%는 넘음).
3. 생성기가 "no shadow" 지시를 어기고 그린 바닥 그림자 중, **부츠에 붙은** 얇은 잔상은
   남아 있다(떨어진 조각은 파이프라인이 제거한다). 게임 크기에서는 사실상 안 보인다.
4. 피격·사망 등 다른 상태 스프라이트는 미착수.
