...
2. F5 로 플레이. 데스크톱에선 마우스 드래그가 터치로 변환돼 조이스틱이 동작하고, 키보드(WASD·방향키)로도 움직인다.
3. 헤드리스 검증·회귀 테스트 목록은 `CLAUDE.md` §3.

## 조작

- 화면을 누르고 드래그 → 가상 조이스틱으로 이동.
- **키보드** `WASD` / 방향키로도 이동한다(데스크톱·웹). 조이스틱과 병행하며, 키를 누르고
  있는 동안에는 키보드가 우선한다 — 마우스로 한 번 누른 뒤 조이스틱에 남은 값에 끌려가지
  않게 하기 위함이다. 입력 액션은 `project.godot` 의 `move_left/right/up/down`.
