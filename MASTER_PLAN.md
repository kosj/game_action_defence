...
| **테마 3종(오브젝트/기믹/보스)** | ✅ 완료 | **Phase 6** — 비주얼셋+선택/해금 + 테마 기믹(가스통·낙석·독가스) + 테마 보스(사냥개·견인체·프라임 변이체) |
...
- **[완료] 6-B — 테마 기믹(필드 위험물):**
  ...
  - **가스통(교외, `GasCan`)** — 좀비 근접/수명에 대폭발(좀비 광역 피해+넉백, 폭심 근처 플레이어 약피해). 유인 처치 도구.
...
- **[남음] 7-B — 밸런스/마감(선택):** 무기 역할 비겹침·30분 곡선 밸런스 패스, 진짜 다단계 보스(체력 구간 페이즈),
  사운드/피드백 마감. (주관·반복 튜닝 영역 — 실제 플레이 데이터 기반 권장.)

### ▶ 스펙 요구사항 구현 완료. 남은 7-B 는 반복 밸런스 튜닝(플레이 기반 권장).
