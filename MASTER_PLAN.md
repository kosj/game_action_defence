...
| **테마 3종(오브젝트/기믹/보스)** | 🟡 부분 | **Phase 6-A** — 교외·도심·연구소 비주얼셋 + 선택/해금(`ThemeData`/`ThemeManager`). 기믹(6-B)·테마보스(6-C) 남음 |
...
- **[남음] 6-B — 테마 기믹(오브젝트):** `gimmick_key` 별 필드 위험물(가스통 폭발/낙석/독가스 웅덩이 등) 스폰·상호작용.
- **[남음] 6-C — 테마 보스:** `boss_key` 별 테마 전용 보스(변이 거대 개/견인 변이체/다단계 변이) — 기존 보스 시스템 확장.

### ▶ 다음: Phase 6-B(테마 기믹) → 6-C(테마 보스), 이후 Phase 7(성능/폴리시)
